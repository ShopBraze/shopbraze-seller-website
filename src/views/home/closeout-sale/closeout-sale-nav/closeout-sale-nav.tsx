...
const CloseOutSaleNav = (props: Props) => {
  return (
      ...
      <div className="flex items-center gap-2 md:gap-4">
        {
          ['All Products', 'T-Shirt', 'Hoodies', 'jacket'].map((item, index) => {
            return (
              <Button className={`${index === 1 ? 'text-gray-1000' : 'text-gray-600'} font-semibold text-[13px] sm:text-sm md:text-base`}>{item}</Button>
            )
          })
        }
      </div>
...
export default CloseOutSaleNav